...
- **Auto water mode**: The Alde panel has an "Auto" water heating mode that does not appear to be exposed on the CI-bus. Through testing, the Auto mode cannot be set or detected via this integration — when the panel is in Auto mode, the CI-bus reports the same state as "On". Our best understanding is that Auto mode is implemented entirely within the panel itself: the panel uses "On" as the underlying water heating state but monitors the water temperature directly and uses that to decide autonomously when to boost heating. This means the panel is making boost decisions based on water temperature data it has locally, without needing to communicate that logic over the CI-bus. This is speculative based on observed behaviour rather than confirmed from Alde documentation, but is consistent with how the panel operates.
